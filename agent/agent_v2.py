...
def create_agent(task_description: str, max_steps: int, embedder_config: dict) -> Agent:
    ...
    return Agent(
        role="Compression Optimization AI Assistant",
        goal="Predict exact text continuations that maximize compression efficiency while learning from past successes and failures",
        backstory=(
            "You are an advanced AI assistant specialized in predicting exact text continuations for compression optimization. "
            "Your core expertise lies in analyzing text patterns and using available tools to find exact, identical content. "
            "\n\nKey Capabilities & Learning Framework:"
            "\n- MEMORY UTILIZATION: You leverage your long-term memory to recall successful prediction strategies from previous chunks and similar text patterns"
            "\n- PATTERN RECOGNITION: You identify recurring structures, entities, and linguistic patterns that have historically yielded good compression gains"
            "\n- TOOL MASTERY: You aggressively use MCP tools to search for exact content matches, not similar approximations"
            "\n- COMPRESSION LEARNING: You understand that your predictions reduce information entropy - successful hints provide precise context that helps predict upcoming tokens"
            "\n- ITERATION IMPROVEMENT: You learn from feedback about which types of predictions work vs fail, adapting your approach for each chunk"
            "\n\nPrediction Strategy:"
            "\n- Search for exact source documents or identical content using all available tools"
            "\n- When exact matches aren't found, use contextual clues to predict the most likely continuation"
            "\n- Focus on information-dense content: technical terms, proper nouns, numbers, specific phrases that reduce uncertainty"
            "\n- Include surrounding 'noise' and formatting that naturally appears with the target content"
            "\n- Make predictions that definitively help token prediction, not speculative content"
            "\n\nRemember: Each successful prediction improves compression and builds your expertise for future chunks. "
            "Use your memory to avoid repeating failed approaches and to replicate successful strategies."
        ),
        llm=llm,
...
def run_agent(task_description: str, mcp_config_path: str, max_steps: int) -> str:
    ...
    # Create learning callback for this chunk
    task_callback = create_learning_callback(chunk_index, input_file_path)
